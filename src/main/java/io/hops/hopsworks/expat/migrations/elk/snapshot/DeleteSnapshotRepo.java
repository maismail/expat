...
public class DeleteSnapshotRepo extends SnapshotRepo {
  private void deleteSnapshotRepo() throws ConfigurationException, NoSuchAlgorithmException, KeyStoreException,
                                           KeyManagementException, IOException, URISyntaxException {
    LOGGER.info("delete elastic snapshot repo");
    setup();
    if (snapshotRepoName == null) {
      throw new ConfigurationException(ExpatConf.ELASTIC_SNAPSHOT_REPO_NAME + " cannot be null");
    }
    ElasticClient.deleteSnapshotRepo(httpClient, elastic, elasticUser, elasticPass, snapshotRepoName);
  }
  @Override
